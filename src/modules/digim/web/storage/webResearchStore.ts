...
export interface StoreContentInput {
  ...
  // Tenancy (migration 007): the console account that gathered this doc and the
  // research (digim_intelligence.id) it belongs to. Null only for legacy/system.
  ownerId?: string | null;
...
export class WebResearchStore {
  ...
  /**
   * Check whether a content hash already exists WITHIN THE SAME (owner, research)
   * — dedup is per-research so one research never reuses another's (or another
   * user's) row. NULL-safe equality (<=>) so legacy/system rows (NULL scope) work.
   * Returns the existing row id or null.
   */
  async findByHash(contentHash: string, ownerId?: string | null, researchId?: string | null): Promise<string | null> {
    try {
      const rows = await DB.query(
        'SELECT id FROM digim_content WHERE content_hash = ? AND owner_id <=> ? AND research_id <=> ? LIMIT 1',
        [contentHash, ownerId ?? null, researchId ?? null],
        true
      );
  ...
  async storeContent(input: StoreContentInput): Promise<StoreContentResult> {
    ...
    const researchId = input.researchId ?? null;
    const existingId = await this.findByHash(input.extracted.contentHash, ownerId, researchId);
    if (existingId) {
    ...
    try {
      await DB.query(
        `INSERT INTO digim_content
           (id, owner_id, research_id, source_id, content_hash, title, content, url, author, published_at,
      ...
      if (/duplicate/i.test(msg)) {
        const raced = await this.findByHash(input.extracted.contentHash, ownerId, researchId);
  ...
  async storeIntelligence(params: {
    ...
    // The id is generated by the caller up front (research()) so content, graph
    // and embeddings can be tagged with it as research_id BEFORE the row exists.
   ...
   */
  async getFreshIntelligence(query: string, level?: string, ownerId?: string | null): Promise<any | null> {
    if (this.cfg.intelligenceCacheTtlHours <= 0) return null;
    // Fail closed: cache is per-owner. Without an owner we never serve a cache
    // hit (a hit would leak another user's result for the same query).
    if (!ownerId) return null;
    try {
      const rows = await DB.query(
        `SELECT * FROM digim_intelligence
         WHERE query_hash = ? AND user_id = ? AND (expires_at IS NULL OR expires_at > NOW())
         ORDER BY generated_at DESC LIMIT 1`,
        [hashQuery(query, level), ownerId],
        true
      );
   ...
   */
  async listIntelligence(opts: { limit?: number; offset?: number; type?: string; search?: string; ownerId?: string | null } = {}): Promise<ResearchSummary[]> {
    // Fail closed: history is per-owner. No owner → no rows.
    if (!opts.ownerId) return [];
    const limit = clampInt(opts.limit ?? 30, 1, 200);
    const offset = clampInt(opts.offset ?? 0, 0, 1_000_000);
    const where: string[] = ['user_id = ?'];
    const args: any[] = [opts.ownerId];
  ...
  async countIntelligence(opts: { type?: string; search?: string; ownerId?: string | null } = {}): Promise<number> {
    if (!opts.ownerId) return 0; // fail closed (per-owner)
    const where: string[] = ['user_id = ?'];
    const args: any[] = [opts.ownerId];
    if (opts.type) { where.push('intelligence_type = ?'); args.push(opts.type); }
    if (opts.search && opts.search.trim()) { where.push('query_text LIKE ?'); args.push(`%${opts.search.trim()}%`); }
  ...
  /** Full stored research record by id (for the detail view), or null. Scoped to
   *  the owner: requesting an id you don't own returns null (prevents IDOR — one
   *  user opening another's research by guessing/enumerating the id). */
  async getIntelligenceById(id: string, ownerId?: string | null): Promise<ResearchRecord | null> {
    if (!ownerId) return null; // fail closed
    try {
      const rows = await DB.query(
        `SELECT * FROM digim_intelligence WHERE id = ? AND user_id = ? LIMIT 1`,
        [id, ownerId],
        true,
      );
  ...
  /** Load stored documents by id, scoped to the owner (defense in depth — even a
   *  content id leaked from another tenant won't resolve). For re-synthesis /
   *  inspection of a research the caller owns. */
  async getContentByIds(ids: string[], ownerId?: string | null): Promise<any[]> {
    if (ids.length === 0 || !ownerId) return [];
    try {
      const placeholders = ids.map(() => '?').join(',');
      const rows = await DB.query(
        `SELECT * FROM digim_content WHERE id IN (${placeholders}) AND owner_id = ?`,
        [...ids, ownerId],
        true
      );
      return Array.isArray(rows) ? rows : [];
