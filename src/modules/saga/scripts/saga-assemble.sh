...
#       [--font /path/to.ttf] [--endcard-sec 2]
#
...
W=1080; H=1920; MUSIC=""; MUSIC_DB=-18; ENDCARD=""; ENDCARD_SEC=2
...
while [ $# -gt 0 ]; do case "$1" in
  ...
  --endcard-sec) ENDCARD_SEC="$2"; shift 2;;
...
while IFS='|' read -r vis secs cap || [ -n "$vis" ]; do
  ...
  echo "file '$out'" >> "$LIST"; echo "  scene $i: $(basename "$vis")  ${secs}s${cap:+  \"$cap\"}"
  i=$((i+1))
done < "$MAN"
...
if [ -n "$ENDCARD" ]; then
  ...
  echo "file '$ec'" >> "$LIST"; echo "  end-card: $(basename "$ENDCARD")  ${ENDCARD_SEC}s"
fi
