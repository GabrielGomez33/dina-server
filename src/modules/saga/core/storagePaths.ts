// File: src/modules/saga/core/storagePaths.ts
// ============================================================================
// DINA SAGA — STORAGE PATH RESOLUTION (security-critical, pure logic)
// ============================================================================
//
// All assets live on the dedicated 4TB SSD under SAGA_ROOT (/mnt/saga).
...
//
// Callers get either a valid absolute path or a thrown SagaPathError.
// ============================================================================

import * as path from 'path';

export class SagaPathError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'SagaPathError';
  }
}
...
function assertSafeId(kind: string, value: string): void {
  if (typeof value !== 'string' || !ID_RE.test(value)) {
    throw new SagaPathError(`Invalid ${kind} id for storage path: ${JSON.stringify(String(value).slice(0, 60))}`);
  }
}

function assertSafeFilename(value: string): void {
  if (typeof value !== 'string' || !FILENAME_RE.test(value) || value.includes('..')) {
    throw new SagaPathError(`Invalid filename for storage path: ${JSON.stringify(String(value).slice(0, 60))}`);
  }
}
...
export class StoragePaths {
  ...
  areaDir(tenantId: string, projectId: string, area: AssetArea): string {
    ...
    if (!sub) throw new SagaPathError(`Unknown asset area: ${String(area)}`);
