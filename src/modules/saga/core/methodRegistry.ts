// File: src/modules/saga/core/methodRegistry.ts
// ============================================================================
// DINA SAGA — DUMP METHOD REGISTRY (pure logic)
// ============================================================================
// The complete DUMP surface of the saga module, with per-method payload
// validation done ONCE at the boundary (reject-don't-guess). The orchestrator's
// `case 'saga'` dispatches by method name; handlers can then trust their
...
export const SAGA_METHODS: readonly MethodSpec[] = Object.freeze([
  // ---- tenancy / projects ----
  {
    method: 'saga_create_tenant',
  ...
  {
    method: 'saga_generate_music_video',
  ...
  {
    method: 'saga_promote_generation',
...
const BY_NAME = new Map(SAGA_METHODS.map((m) => [m.method, m]));

export function isSagaMethod(method: string): boolean {
...
export function validatePayload(method: string, data: unknown): ValidationResult {
  ...
  if (!spec) return { valid: false, errors: [`Unknown saga method: ${method}`] };
