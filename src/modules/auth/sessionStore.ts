...
// ============================================================================
// One row per device login in `user_sessions` (shared with mirror-server). The
// JWT sessionId maps to a row here; a session is valid only while the row is
// unexpired and not revoked. This is what makes logout instant — we don't wait
// for the JWT to expire, we flip `revoked` and every future validate() fails.
//
// Schema (mirror-server-compatible): user_id, session_id, user_agent,
// ip_address, device_fingerprint, revoked(TINYINT), revoked_at, expires_at,
// created_at. See migrations/004_auth_users.ts.
// ============================================================================
...
export async function markSessionUsed(userId: string, sessionId: string): Promise<void> {
  ...
  try {
    await q(
      `UPDATE user_sessions SET expires_at = expires_at WHERE user_id = ? AND session_id = ?`,
      [userId, sessionId],
    );
  } catch {
    /* non-fatal */
