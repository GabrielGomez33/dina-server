# Goal #2 — `dina-visuals` Foundation Slice (multi-tenant image/video generation)
...
├── README.md                                   ← this file
└── dina-server/
    ├── migrations/003_visuals_core.ts          ← full multi-tenant schema (7 tables, idempotent)
    ├── src/modules/visuals/
    │   ├── index.ts                            ← VisualsModule (DUMP entry, mirror-pattern singleton)
    │   ├── visualsRoutes.ts                    ← HTTP→DUMP boundary (truthStreamRoutes pattern)
    ...
    ├── test/visuals/visualsFoundationTest.ts   ← hermetic proofs — 97 assertions, all green
...
| Proven mirror tactic | visuals implementation |
|---|---|
| HTTP → `createDinaMessage` → `dina.handleIncomingMessage` | `visualsRoutes.dispatch()` — one uniform path for all 13 endpoints |
...
| orchestrator `case 'mirror'` → method switch | additive `case 'visuals'` → `handleVisualsMessage` |
| per-module error isolation (`MirrorModuleError`) | `VisualsModuleError` + every handler failure returned as a structured `{code,message}` — proven in test 5i |
...
```bash
cd goal-2-dina-visuals/dina-server
npx tsc --noEmit -p tsconfig.json                       # strict type-check — clean
npx ts-node test/visuals/visualsFoundationTest.ts       # RESULTS: 97 passed, 0 failed
```
...
interactive chat always holds priority. `visuals_jobs.priority` carries the tenant's `gpu_priority`
so the paid-tier queue-jump hook is already in place.

## Schema (migration 003) — locked decisions encoded

`visuals_tenants` (plan free/pro/admin, quota_bytes, gpu_priority) → `visuals_memberships`
(owner/editor/viewer, unique per tenant+user, keyed to `dina_users.dina_key`) → `visuals_projects`
(**per-tenant slug uniqueness** — no global collisions, soft-delete, bytes_used) → `visuals_manifests`
(versioned, draft/frozen/retired) · `visuals_audio_tracks` (analysis cached forever in
`analysis_json`) · `visuals_generations` (seed + `parent_generation_id` lineage, `promoted`
TTL-immunity, `ttl_expires_at` with janitor index) · `visuals_jobs` (all 8 queue classes, progress
...
4. **mirror-server frontend** — staged under `goal-2-dina-visuals/mirror-server/` when we get there.
