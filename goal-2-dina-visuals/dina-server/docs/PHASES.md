...
`VISUALS_ROOT`; no code change required (StoragePaths is root-agnostic and traversal-proofed).
...
| **1** | Backend integration | GPU arbiter wired, migration 003, visuals module + routes live | `/dina/visuals/status` green, 97+29 proofs green in-tree |
...
`VISUALS_ROOT: '/mnt/nvme_tugrrstorage2/Dina/SAGA'`
...
`sha256sum <file>` + source URL + license. (This file seeds the visuals_models registry in Phase 2.)
...
- [ ] SAGA tree exists on the 4TB NVMe, owned by the dina user, `VISUALS_ROOT` staged in PM2 env
