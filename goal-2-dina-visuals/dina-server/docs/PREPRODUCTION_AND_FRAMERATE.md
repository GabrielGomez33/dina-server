# dina-visuals — Pre-Production Pipeline & Frame-Rate Policy (locked design)

This document extends the foundation slice (migration 003, `VisualsModule`) with the film
...
| Film artifact | dina-visuals implementation |
|---|---|
...
| **Storyboard** (camera angles, character positions, movement, dialogue/action notes, transitions) | `visuals_storyboard_panels` rows — each panel is a cheap draft image generation PLUS structured fields for angle/movement/notes; ordered per shot |
| **Shot list** (every shot, lens, movement, setup) | `visuals_shots` — the machine-readable contract. "Lens/setup" become prompt+seed+LoRA-set metadata; this table is what the ShotPlanner consumes, so the shot list literally drives generation |
...
```
visuals_scenes            id, project_id FK, seq, title, synopsis, status(draft/approved/superseded),
                          soft-delete + timestamps
visuals_shots             id, scene_id FK, shot_number, shot_type ENUM(standard, holdLoop, chained),
...
visuals_storyboard_panels id, shot_id FK, seq, generation_id FK (the panel image),
                          notes TEXT, status(draft/approved/superseded)
ALTER visuals_generations ADD asset_category ENUM(production, concept_location, concept_character,
                          concept_prop, concept_costume, concept_mood, storyboard, animatic) DEFAULT 'production'
ALTER visuals_generations MODIFY kind ENUM(..., 'animatic')   -- additive enum extension
```
...
```
visuals_create_scene            {tenantId, projectId, title, synopsis?}
visuals_create_shot             {tenantId, projectId, sceneId, spec}       // spec = shot-list fields
visuals_generate_storyboard     {tenantId, projectId, shotId, prompt, panels?}   // draft-tier images
visuals_generate_concept        {tenantId, projectId, category, prompt, params?}
visuals_assemble_animatic       {tenantId, projectId, sceneId, audioTrackId?, scratchDialogue?}
visuals_approve                 {tenantId, projectId, kind, id}            // uniform stage-gate approval
visuals_get_scene_board         {tenantId, projectId, sceneId}             // scene + shots + panels + animatic
```
