# dina-visuals — Readiness Plan ("sharpen the sword")
...
**Build items**
- `visuals_models` registry table: name, family, version, sha256, source URL, disk path, size,
...
**Build items**
- One-time calibration suite (`ops/calibrate-visuals.sh` + a small runner job): measures s/frame at
  480p/720p per video model, model load seconds, upscale fps, RIFE fps, TTS chars/sec, on THIS
  hardware. Results stored in `visuals_calibration` and consumed by the ETA estimator that feeds
  `JobProgress.eta_seconds`.
- Golden-path smoke test: tiny end-to-end render (image → 1s video → upscale → interpolate →
  assemble) as the post-deploy health check; wired next to `npm run test:visuals` as `test:visuals:gpu`
...
**Build items**
- `gpu_seconds` on `visuals_jobs` (arbiter lease hold time — already measured, just record it);
...
**Build items**
- `visuals_scripts` table (project-scoped, versioned text) + `visuals_breakdown_script` DUMP method:
...
- Output lands as `draft` rows in visuals_scenes/visuals_shots — the human edits and **approves**;
